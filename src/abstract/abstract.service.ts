...
export abstract class AbstractService<T extends AbstractModel> {
	...
	async find(
		...
		index?: string,
		startId?: Types.ObjectId | null,
	): Promise<T[]> {
		let data;
		try {
			const filter = startId ? { _id: { $gt: startId }, ...filterQuery } : filterQuery;
			const query = this.model.find(filter, projection, options).lean();
			if (index) query.hint(index);
			data = await query;
		} catch (e) {
			throw new InternalServerErrorException(e.message);
		}
	...
	async findByIdAndUpdate(
		...
		let data;
		try {
			data = await this.model.findByIdAndUpdate(id, updateQuery, options).lean();
		} catch (e) {
			if ((e as MongoError) && e.code === 11000) {
				throw new ConflictException(
					`The provided data for ${this.modelName} triggered a duplicate error. Probably, this model has an extra unique field other its id.`,
	...
	async findOneAndUpdate(
		...
		let data;
		try {
			data = await this.model.findOneAndUpdate(filterQuery, updateQuery, options).lean();
		} catch (e) {
			if ((e as MongoError) && e.code === 11000) {
				throw new ConflictException(
					`The provided data for ${this.modelName} triggered a duplicate error. Probably, this model has an extra unique field other its id.`,
	...
	async updateOne(
		...
		try {
			return await this.model.updateOne(filter, update, options).lean();
		} catch (e) {
			if ((e as MongoError) && e.code === 11000) {
				throw new ConflictException(
					`The provided data for ${this.modelName} triggered a duplicate error. Probably, this model has an extra unique field other its id.`,
	...
	async updateMany(
		...
		try {
			return await this.model.updateMany(filter, update, options).lean();
		} catch (e) {
			if ((e as MongoError) && e.code === 11000) {
				throw new ConflictException(
					`The provided data for ${this.modelName} triggered a duplicate error. Probably, this model has an extra unique field other its id.`,
	...
	async findByIdAndDelete(id: Types.ObjectId): Promise<T> {
		let data;
		try {
			data = await this.model.findByIdAndDelete(id).lean();
		} catch (e) {
			throw new InternalServerErrorException(e.message);
		}
		if (!data) {
	...
	async findOneAndDelete(filterQuery: FilterQuery<T>): Promise<T> {
		let data;
		try {
			data = await this.model.findOneAndDelete(filterQuery).lean();
		} catch (e) {
			throw new InternalServerErrorException(e.message);
		}
		if (!data) {
